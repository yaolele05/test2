...
void path(const char*name,const char*oripath,char*goal)
{
    ...
    if (oripath != NULL && name != NULL && goal != NULL)
    {
    strcpy(goal,oripath);
 ...
 int compare_dir(const void *a, const void *b)
{
    ...
    int m1 = (strcmp(m->name, ".") == 0);
    int m2 = (strcmp(m->name, "..") == 0);
    int n1  = (strcmp(n->name, ".") == 0);
    int n2 = (strcmp(n->name, "..") == 0);
...
DIR *dir = opendir(lu);
        ...
        size_t ca=256;
        struct Dirdetail *alldir = malloc(ca * sizeof(struct Dirdetail));
        if (alldir==NULL) 
        {
        perror("malloc");
        closedir(dir);
         ...
         while (filr != NULL)
        {
            if(filr->d_name[0] == '.' && _a == 0)
            {
             ...
             if (count >= ca) {
            size_t nca = ca * 2;
            struct Dirdetail *tp = realloc(alldir, nca * sizeof(struct Dirdetail));
            if (tp==NULL) {
                perror("realloc");
                free(alldir);  
              closedir(dir);
           ...
           for(int i=0;i<count;i++)
           {
            ...
            if (_R && S_ISDIR(f->at.st_mode) && 
             strcmp(f->name, ".") != 0 && strcmp(f->name, "..") != 0) 
             {
               ...
               if (f->allpath == NULL)
               return;
               snprintf(subb,sizeof(subb),"%s",f->allpath);
             printf("\n%s:\n",subb);
             prrri(subb);
         }
           }
